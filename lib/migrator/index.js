...
var Migrator = module.exports = function(app) {
  if (this === global) {
    ...
    return instance = new Migrator(require('../app')());
  }

  var adapter = Adapter(app.config.database)
    , parser = Parser(adapter.name)
    , directory = path.join(app.root, app.config.path.db.migrate);
  ...
  var run = function(conn, migrations, verb, callback) {
    ...
    var m = migrations.shift()
      , reporter = new Reporter(verb == 'up'? 'migrate' : 'rollback', m.name)
      , queries = m[verb]();
    ...
    if (verb === 'up') schema.insert(conn, m.version);
    else schema.remove(conn, m.version);
